...
class _CarServiceSearchScreenState extends State<CarServiceSearchScreen> with AutomaticKeepAliveClientMixin {
        ...
        body: SafeArea(
          child: Consumer<CarServicesProvider>(
            builder: (context, provider, child) {
              ...
              final premiumStarAds = allAds.where((ad) => CarServiceAdCardAdapter(ad).priority == AdPriority.PremiumStar).toList();
              final premiumAds = allAds.where((ad) => CarServiceAdCardAdapter(ad).priority == AdPriority.premium).toList();
              final featuredAds = allAds.where((ad) => CarServiceAdCardAdapter(ad).priority == AdPriority.featured).toList();
              final freeAds = allAds.where((ad) => CarServiceAdCardAdapter(ad).priority == AdPriority.free).toList();
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 3.h),
                                Center(child: Text(s.carservices, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 24.sp, color: KTextColor))),
                              ],
                            ),
                          ),
                          SizedBox(height: 8.h),
                          ...
                          if (provider.isLoading && allAds.isEmpty)
                            ...
                            Center(child: Padding(padding: const EdgeInsets.all(32.0), child: Text('Error: ${provider.error}')))
                             ...
                             Center(child: Padding(padding: const EdgeInsets.all(32.0), child: Text("s.no_result_found")))
                          else ...[
                        ...
                        child: Column(
                          children: [
                             GestureDetector(onTap: () {
                               ...
                               final infoProvider = Provider.of<CarServicesInfoProvider>(context, listen: false);
                               infoProvider.updateSelectedEmirate(null);
                               infoProvider.updateSelectedServiceType(null);
                               
                               final provider = Provider.of<CarServicesProvider>(context, listen: false);
                               provider.clearPriceFilters();
                               
                               context.pop();
                             }, child: Row(children: [Icon(Icons.arrow_back_ios, color: KTextColor, size: 17.sp), Transform.translate(offset: Offset(-3.w, 0), child: Text(s.back, style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w500, color: KTextColor)))])),
                             SizedBox(height: 8.h),
                             _buildFiltersRow(s),
                             SizedBox(height: 4.h),
                             _buildAdHeader(context, allAds.length),
          ...
          Expanded(child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(child: Text(S.of(context).sort, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w600, color: KTextColor, fontSize: 12.sp))),
                  SizedBox(
...
Widget _buildMultiSelectField(BuildContext context, String title, List<String> selectedValues, List<String> allItems, Function(List<String>) onConfirm, {bool isFilter = true}) {
    ...
    return GestureDetector(
      ...
      child: Container(height: isFilter ? 35 : 48, width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 8), alignment: Alignment.center, 
...
Widget _buildRangePickerField(BuildContext context, {required String title, String? fromValue, String? toValue, required String unit, required VoidCallback onTap, bool isFilter = true}) {
    ...
    return GestureDetector(
      onTap: onTap,
      child: Container(
        height: isFilter ? 35 : 48, width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 8), alignment: Alignment.center,
        decoration: BoxDecoration(color: Colors.white, border: Border.all(color: borderColor), borderRadius: BorderRadius.circular(8)),
        child: Text(displayText, style: TextStyle(fontWeight: FontWeight.w500, color: KTextColor, fontSize: 9.5), overflow: TextOverflow.ellipsis, maxLines: 1),
      ),
    );
...
Future<Map<String, String?>?> _showRangePicker(BuildContext context, {required String title, String? initialFrom, String? initialTo, required String unit}) {
    return showModalBottomSheet<Map<String, String?>>(context: context, backgroundColor: Colors.white, isScrollControlled: true, shape: RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
    ...
    return Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom, left: 16, right: 16, top: 16),
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            TextButton(onPressed: () { _fromController.clear(); _toController.clear(); setState(() {}); }, child: Text(s.reset, style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 14.sp))),
          ]),
          SizedBox(height: 16.h),
          Row(children: [ buildTextField(s.from, widget.unit, _fromController), Padding(padding: const EdgeInsets.symmetric(horizontal: 8.0), child: Text(s.to, style: const TextStyle(fontWeight: FontWeight.w600, color: KTextColor, fontSize: 14))), buildTextField(s.to, widget.unit, _toController)]),
