...
class RealEstateAdScreen extends StatefulWidget {
  ...
  const RealEstateAdScreen({Key? key, required this.onLanguageChange}) : super(key: key);
...
class _RealEstateAdScreenState extends State<RealEstateAdScreen> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Center(
                child: Text(s.realEstateAds, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 24.sp, color: KTextColor)),
              ),
              SizedBox(height: 10.h),

              // +++ بداية التعديلات على حقول النموذج +++
              _buildFormRow([
                _buildSingleSelectField(context, s.emirate, selectedEmirate, ['Dubai', 'Abu Dhabi', 'Sharjah'], (selection) => setState(() => selectedEmirate = selection)),
                _buildSingleSelectField(context, s.district, selectedDistrict, ['Jumeira', 'Al Barsha', 'Downtown'], (selection) => setState(() => selectedDistrict = selection)),
              ]),
              const SizedBox(height: 7),

              _buildFormRow([
                _buildTitledTextFormField(s.area, s.area, _areaController, borderColor, currentLocale),
                _buildTitledTextFormField(s.price, '120,000', _priceController, borderColor, currentLocale, isNumber: true),
              ]),
              const SizedBox(height: 7),

              _buildFormRow([
                _buildSingleSelectField(context, s.contractType, selectedContractType, ['Rent', 'Sale'], (selection) => setState(() => selectedContractType = selection)),
                _buildSingleSelectField(context, s.propertyType, selectedPropertyType, ['Villa', 'Apartment', 'Land'], (selection) => setState(() => selectedPropertyType = selection)),
              ]),
              const SizedBox(height: 7),
              
              _buildTitleBox(context, s.title, 'Luxury Villa For Monthly Rent', borderColor, currentLocale),
              const SizedBox(height: 7),
              
              TitledSelectOrAddField(
                title: s.advertiserName, 
                value: selectedAdvertiserName, 
                items: ['Dubai Investments', 'Emaar Properties'], 
              ...
              TitledDescriptionBox(title: s.description, initialValue: 'Amazing Villa With Luxury Viewing', borderColor: borderColor, maxLength: 15000),
              const SizedBox(height: 10),
              
              _buildImageButton(s.addMainImage, Icons.add_a_photo_outlined, borderColor),
              const SizedBox(height: 7),
              _buildImageButton(s.add9Images, Icons.add_photo_alternate_outlined, borderColor),
              const SizedBox(height: 7),
              ...
              Directionality(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8.w),
                    Expanded(
                      child: Text('Dubai Jumeira', style: TextStyle(fontSize: 14.sp, color: KTextColor, fontWeight: FontWeight.w500)),
                    ),
                  ],
                ),
              ),
  ...
  // --- دوال المساعدة المنقولة من شاشة السيارات ---
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ++++        الودجت المساعدة المنقولة من شاشة السيارات     ++++
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
