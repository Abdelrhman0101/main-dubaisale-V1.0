...
class CarServicesSaveAdScreen extends StatelessWidget {
  // استقبال دالة تغيير اللغة
  final Function(Locale) onLanguageChange;

  const CarServicesSaveAdScreen({Key? key, required this.onLanguageChange})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    const districtValue = 'Ras Alkhor';
    const serviceTypeValue = 'Garages';
          ...
          child: Column(
            ...
            children: [
              ...
              _buildFormRow([
                ...
                _buildTitledDropdownField(
                    context, s.district, ['Ras Alkhor', 'Al Barsha'], districtValue, borderColor),
                _buildTitledTextField(s.area, 'Industrial Area 2', borderColor, currentLocale),
              ]),
              const SizedBox(height: 7),

              _buildFormRow([
                _buildTitledDropdownField(
                    context, s.serviceType, ['Garages', 'Cleaning'], serviceTypeValue, borderColor),
                _buildTitledTextField(s.serviceName, 'Change Oil', borderColor, currentLocale),
                _buildTitledTextField(s.price, 'AED 300', borderColor, currentLocale, isNumber: true),
              ]),
              const SizedBox(height: 7),

              _buildTitleBox(context, s.title, 'Change Oil With Good Quality', borderColor, currentLocale),
              const SizedBox(height: 7),

              TitledTextFieldWithAction(title: s.advertiserName, initialValue: 'Future Garage', borderColor: borderColor, onAddPressed: (){ print("Add advertiser clicked"); }),
              const SizedBox(height: 7),
              ...
              TitledDescriptionBox(title: s.description, initialValue: 'Very Specifically Garage With Smart Engineering Equipment', borderColor: borderColor, maxLength: 1500),
