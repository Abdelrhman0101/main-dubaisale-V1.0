...
class _OtherServiceSearchScreenState extends State<OtherServiceSearchScreen>
              ...
              children: [
                Flexible(
                  ...
                  child: _buildMultiSelectField(
                    ...
                    const ["Contracting", "Logistics", "Events"],
                ...
                Flexible(
                  flex: 3,
                  child: _buildMultiSelectField(
                    context, S.of(context).service, _selectedServices,
                    const ["Cleaning", "Maintenance", "Coordination"],
                    (selection) => setState(() => _selectedServices = selection), isFilter: true
                  ),
                ),
                SizedBox(width: 3.w),
                Flexible(
  ...
  Widget build(BuildContext context) {
    ...
    final premiumStarAds = OtherServiceDammyData.where((j) => j.priority == AdPriority.PremiumStar).toList();
    final premiumAds = OtherServiceDammyData.where((j) => j.priority == AdPriority.premium).toList();
    final featuredAds = OtherServiceDammyData.where((j) => j.priority == AdPriority.featured).toList();
    final freeAds = OtherServiceDammyData.where((j) => j.priority == AdPriority.free).toList();
  ...
  Widget _buildCard(FavoriteItemInterface item) {
    return GestureDetector(
      onTap: () {
        context.push('/other_service-details', extra: item);
      },
