...
class _ElectronicSearchScreenState extends State<ElectronicSearchScreen>
 ...
 void _handleScroll() {
              ...
              children: [
                Flexible(
                  ...
                  child: _buildRangePickerField(
                    context, 
                    title: S.of(context).price, fromValue: _priceFrom, toValue: _priceTo, unit: "AED", isFilter: true,
                    onTap: () async {
                ...
                Flexible(
                  ...
                  child: _buildMultiSelectField(
                    context, 
                    S.of(context).section, _selectedSections, 
                    const ["Mobiles", "Screens", "Appliances"],
                    (selection) => setState(() => _selectedSections = selection), 
                    isFilter: true
                  ),
                ),
                SizedBox(width: 4.w),
                Flexible(
                  ...
                  child: _buildMultiSelectField(
                    context, 
                    S.of(context).product, _selectedProducts, 
                    const ["New", "Used"],
                    (selection) => setState(() => _selectedProducts = selection), 
                    isFilter: true
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    ...
    final premiumStarAds = ElectronicDummyData.where((c) => c.priority == AdPriority.PremiumStar).toList();
    final premiumAds = ElectronicDummyData.where((c) => c.priority == AdPriority.premium).toList();
    final featuredAds = ElectronicDummyData.where((c) => c.priority == AdPriority.featured).toList();
    final freeAds = ElectronicDummyData.where((c) => c.priority == AdPriority.free).toList();
  ...
  Widget _buildCard(FavoriteItemInterface item) {
    return GestureDetector(
      onTap: () {
        context.push('/electronic-details', extra: item);
      },
      child: Directionality(
        textDirection: TextDirection.ltr,
        child: SearchCard(
          showLine1: false, item: item, showDelete: false, onAddToFavorite: () {},
          onDelete: () { setState(() {ElectronicDummyData.remove(item);});},
        ),
      ),
    );
  }
